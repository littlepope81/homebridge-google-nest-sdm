...
exports.describeProbeFailure = exports.probeFfmpeg = exports.resolveFfmpegPath = exports.PROBE_TIMEOUT_MS = void 0;
 ...
 */
function resolveFfmpegPath(config, bundledResolver = defaultBundledResolver) {
    var _a;
    // Trim and reject whitespace-only, so a blank field in the config UI behaves as
    // "not set" rather than spawning ''.
    const configured = (_a = config === null || config === void 0 ? void 0 : config.videoProcessor) === null || _a === void 0 ? void 0 : _a.trim();
    if (configured)
        return configured;
 ...
 */
function probeFfmpeg(ffmpegPath, timeoutMs = exports.PROBE_TIMEOUT_MS) {
    return new Promise(resolve => {
        (0, child_process_1.execFile)(ffmpegPath, ['-hide_banner', '-encoders'], { timeout: timeoutMs }, (error, stdout, stderr) => {
            var _a;
            if (error) {
                ...
                return resolve({ ok: false, kind: 'exit_error', detail: String((_a = error.message) !== null && _a !== void 0 ? _a : error) });
            }
            // -encoders writes to stdout, but tolerate builds that report on stderr.
            const output = `${stdout !== null && stdout !== void 0 ? stdout : ''}${stderr !== null && stderr !== void 0 ? stderr : ''}`;
