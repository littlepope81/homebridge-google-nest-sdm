...
import {RtpPacketFilter} from "./RtpPacketFilter";
...
export abstract class NestStreamer {
    ...
    protected teardownRequested = false;
    ...
    protected async stopAssignedStream(): Promise<void> {
    ...
    protected async rejectIfTeardownRequested(): Promise<void> {
...
export class WebRtcNestStreamer extends NestStreamer {
    ...
    private firSeq = 0;
    private readonly videoRtpFilter = new RtpPacketFilter();
    ...
    async initialize(): Promise<NestStream> {
        ...
        videoTransceiver.onTrack.subscribe((track) => {
            ...
            track.onReceiveRtp.subscribe((rtp) => {
                const decision = this.videoRtpFilter.inspect(
                    rtp.header.ssrc,
                ...
                if (decision.gapSize > 0
                    ...
                    this.log.debug(
                        `Video RTP sequence gap detected `
                        + `(seq=${rtp.header.sequenceNumber}, missing=${decision.gapSize}, `
                        + `gapEvents=${stats.gapEvents}).`,
                        this.displayName
                    );
                }
