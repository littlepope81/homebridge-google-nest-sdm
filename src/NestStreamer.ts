...
import {RTCPeerConnection, RTCRtpCodecParameters, RtcpPayloadSpecificFeedback, ReceiverEstimatedMaxBitrate, RTCRtpTransceiver, MediaStreamTrack} from "werift";
...
export class WebRtcNestStreamer extends NestStreamer {
    ...
    // Set when the video track arrives; cleared on teardown so a late RECONFIGURE
    // cannot send RTCP over a closed transport.
    ...
    private sendRemb() {
        if (!this.videoTransceiver || !this.videoTrack) return;
        let exp = 0, mantissa = this.rembBitrate;
        while (mantissa > 0x3ffff) { mantissa = Math.floor(mantissa / 2); exp++; }
        try {
...
a=sendrecv`
    ...
    setMaxBitrate(bitrate: number): void {
        ...
        const clamped = Math.max(bitrate, 300_000);
